...
| ファイル | 内容 | 件数 |
|---|---|---|
| `data/iki-seido-org.json` | 全データ(組織+分掌事務+改正沿革+メタ情報)。**唯一の正** | ─ |
| `data/org_units.csv` | 組織ユニット(部・課・班・室・出先機関)の一覧 | 96件 |
| `data/duties.csv` | 分掌事務(条文の号単位) | 597件 |
| `data/amendments.csv` | 改正沿革(制定含む) | 57件 |
| `data/synonyms.yml` | 同義語辞書(市民の言葉→条文の言葉。条文由来ではない) | 71件 |
| `data/scenarios.yml` | ライフイベント(逆引き検索の入口。条文由来ではない) | 8件 |
| `datapackage.json` | Frictionless Data 形式のデータセット定義 | ─ |
| `tools/build.mjs` | `data/iki-seido-org.json` から各ビューのデータ節と CSV を生成するスクリプト | ─ |
| `scripts/parse_reiki.py` | 本番用パイプラインの骨格(例規HTML→同スキーマ)。v0.2で実装 | ─ |

組織ユニットの内訳: 部6 / 課24 / 班59 / 室1 / 出先機関6(支所4・福祉事務所・事務所グループ)。

**`org_units.csv`・`duties.csv`・`amendments.csv` は `data/iki-seido-org.json` からの派生物です。**
直接編集せず、JSON を直して `node tools/build.mjs` を回してください
（`scripts/build_dataset.js` は v0.1 当時の生成経路で、現在は使いません）。
...
- 支所配下の班(u078・u080・u081・u083・u084・u086・u087)と事務所グループ(u089)の
  分掌事務は概要のみ(原文は支所だけで約69号を列挙)。
- 会計課(u075)・会計係(u076)の分掌事務は未収録(`duty_count` が 0)。概要すら入っておらず、
  抽出が未着手。「概要のみ」の支所・事務所とは意味が違う(存在に気づけない)。
