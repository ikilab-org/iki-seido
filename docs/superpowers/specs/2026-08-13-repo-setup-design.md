...
- `datapackage.json` の壊れたリソースパスの修正（§5.2）
- `quality_notes` の会計課に関する記述の訂正（§7.2）
...
| error | `datapackage.json` の `resource.path` が実在しない（§5.2） |
...
### 5.2 Data Package のリソースパス

現行の `data/datapackage.json` は4つのリソースすべてで `path` を
`data/org_units.csv` のように書いている。Data Resource 仕様ではローカルの
`path` は **descriptor 自身の位置からの相対パス**として解決されるため、
`data/datapackage.json` に置かれたこの記述は `data/data/org_units.csv` を
指す。存在しないパスであり、公開した Data Package は読み込めない。

`path` から `data/` を落とし、`org_units.csv` / `duties.csv` /
`amendments.csv` / `iki-seido-org.json` に修正する。

あわせて validate で**全リソースの `path` が実在するか**を検査する。フィールド
定義とヘッダの一致（既存の検査）だけでは、パスが壊れていても通ってしまい、
「読み込めない Data Package を公開している」状態を検出できない。この検査は
将来ファイル名を変えたときにも効く。

参照: [Data Resource — Frictionless Data](https://specs.frictionlessdata.io/data-resource/)
...
ただし**「597件はすべて権利の対象外」とは書かない。** 一部の分掌事務は原文の
列挙をそのまま写したものではなく IKILAB が要約したもので（§7.2 の既知の限界1）、
要約・選択・編集まで一律に13条の対象外と言い切ることはできない。§7.2 の3区分と
揃えて、次のように限定する。
...
どのレコードが要約にあたるかは `NOTICE.md` にユニットID単位で列挙する。
現行データを実際に当たったところ、要約されているのは**支所本体ではなくその配下の
班**で、次の8件である。
...
支所本体（u077・u079・u082・u085）は `duty_count` が 0 で、要約を含まない。
郷ノ浦支所だけ班が1つなのは地域班を置かないためで、データの欠落ではない
（u077 の `note` に根拠がある）。

**要約扱いから外すもの2件。**

- **福祉事務所（u088）** — 唯一の分掌事務は「社会福祉法第14条第6項に規定する
  事務」で、これは条文の転記であって要約ではない。既知の限界1にも挙がっていない
- **会計課（u075）・会計係（u076）** — `duty_count` が 0 で、**分掌事務が
  1件も収録されていない**。「概要のみ」ではなく未収録である

最後の点は `quality_notes` の記述（「支所・事務所・会計課の分掌事務は概要のみ」）と
実データの食い違いにあたる。`DISCLAIMER.md` の既知の限界1では、会計課を
「概要のみ」ではなく「未収録」として正しく書き分ける。データ側の
`quality_notes` の文言も同時に直す。

レコード単位で機械判別できるフラグを持たせるかはスキーマ変更になるため
§10 の申し送りとする。
...
既知の限界を「未確認」として名指しで書く。`quality_notes` は1件目を
「支所・事務所・会計課の分掌事務は概要のみ」とまとめているが、§7.1 で実データに
当たったところ**会計課だけは概要ですらなく未収録**だった。両者は利用者にとって
意味が違う（概要は読めるが不正確、未収録は存在に気づけない）ので書き分ける。

1. 支所配下の班と事務所の分掌事務は概要のみ（原文は支所だけで約69号。
   該当する8件のユニットIDは §7.1 の表）
2. **会計課・会計係の分掌事務は未収録**（`duty_count` が 0）
3. 教育委員会規則の分掌事務の条番号が未確定
4. 沿革の `recorded_on` は公布日と施行日が混在している可能性
5. 令和8年3月19日条例第4号は沿革にあるが本文ページに附則が見当たらない

`data/iki-seido-org.json` の `quality_notes` も同じ書き分けに直す。
