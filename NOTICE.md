...
**分掌事務の文言を引用する場合は、それが転記か要約かで分かれます。**
記述の中に「概要」の語が入っているもの（`(概要)`、
`(原文は約69項目を列挙 — モックでは概要のみ)` など）が上表の8ユニットの
要約で、条文そのものではないため、組織図と同じクレジットが要ります:
...
「概要」の語が入っていない記述は条文の転記なので、クレジットは不要です。
見分けがつかないときは、原文（例規本文）にあたってください。
